...
//    Tu = normalize (mix(dPdv,-vector(-Nn),bendTangentAmt)),   /* bent tangent up along length of hair */
  ...
  vector Tu = normalize (mix(dPdv,-vector(V),bendTangentAmt));   /* bent tangent up along length of hair */
...
illuminance ("-envLight",P, hair_normal, PI) {
        ...
        if(nondiffuse < 1 && category!="envLight"){
          //DIFFUSE LOOP
          //OLD_____  hair_diffuse += Cl * atten * (1 - nondiffuse) ;
          //REMOVING DUE TO SHAVE 7 NEW N export
          float rawDiff, rawDiffu;
          color Cselfshad;
          Cselfshad = Cl;
          rawDiff =  sqrt(1 - pow((T . Ln), 2));
          rawDiffu =  sqrt(1 - pow((Tu . Ln), 2));
          diffK = rawDiff;
          hair_diffuse += diffK * Cselfshad;
